...
@Controller
@RequestMapping("/gestion/resultado")
...
public class ResultadoController implements GenericController {
	...
	static final Logger LOGGER = LoggerFactory
			.getLogger(ResultadoController.class);

	private static final String MODEL = "Test";
	private static final String BASE = "/gestion/resultado";
	...
	public String list(ModelMap model) {
		List<Test> objetos = testService.findAll();
		model.addAttribute("objetos", objetos);
		return "Resultado" + LIST;
	}

	/*
	 * @RequestMapping(method = RequestMethod.GET) public String list(ModelMap
	 * model) { Usuario objeto =
	 ...
	 * model.addAttribute(OBJETO, objeto); return "Resultado" + LIST; }
	 */
	...
	/*
	 * @RequestMapping(value = { "/{test}/{pregunta}" }, method =
	 * RequestMethod.POST) public String guardarPregunta(@RequestParam Respuesta
	 * respuesta,
	 * 
	 * @PathVariable("test") String test,
	 * 
	 * @PathVariable("pregunta") String pregunta, ModelMap model,
	 * RedirectAttributes attributes) { try { Test testActual =
	 * testService.getByCodigo(test); Pregunta preguntaActual =
	 * preguntaService.getByCodigo(pregunta); Usuario usuario =
	 * service.getByCodigo(controller .getCurrentUserCode());
	 * 
	 * // Crea los datos del nuevo intento on los datos suministrados y lo //
	 * actualiza Intento intento = new Intento();
	 * intento.setTest(testService.getByCodigo(test));
	 * intento.setPregunta(preguntaActual); intento.setUsuario(usuario);
	 * intento.setAcierto(respuesta.getCorrecta()); intento.setIntentos(0); //
	 * Será sobreescrito intentoService.update(intento);
	 * 
	 * // Busca la pregunta siguiente del test o termina
	 * 
	 * // Ordena la lista de preguntas por ID Set<Pregunta> preguntas =
	 * testActual.getPreguntas(); List<Pregunta> list = new
	 * ArrayList<Pregunta>(preguntas); Collections.sort(list, new
	 * Comparator<Pregunta>() {
	 * 
	 * @Override public int compare(Pregunta p1, Pregunta p2) { return
	 * p1.getId() - p2.getId(); } });
	 * 
	 * int indice = 0; for (Pregunta p : list) { if
	 * (p.getId().equals(preguntaActual.getId())) { break; } indice++; }
	 * LOGGER.info("Indice : {}", indice); try { Pregunta preguntaSiguiente =
	 * list.get(indice + 1); return "redirect:" + BASE + "/" +
	 * testActual.getCodigo() + "/" + preguntaSiguiente.getCodigo(); } catch
	 * (IndexOutOfBoundsException e) { return "redirect:" + BASE; }
	 * 
	 * } catch (Exception e) { Intento objeto = intentoService.getIntento(test,
	 * pregunta, controller.getCurrentUserCode()); model.addAttribute(EDIT,
	 * true); model.addAttribute(OBJETO, objeto); model.addAttribute(ERROR,
	 * e.getMessage()); return "IntentoPregunta" + FORM; }
	 * 
	 * }
	 */
	...
	public String getPath() {
		return ResultadoController.BASE;
	}
